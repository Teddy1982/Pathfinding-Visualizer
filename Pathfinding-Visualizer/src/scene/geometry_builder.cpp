...
void GeometryBuilder::createPathConnection2D(
	glm::vec2 a,
	glm::vec2 b,
	float thickness,
	bool isLeftViewport)
{
	glm::vec2 forward = b - a;
	float length = glm::length(forward);
	...
	float halfThickness = thickness * 0.5f;
	glm::vec2 r = right * halfThickness;

	constexpr float z = 0.01f;

	glm::vec3 p0(a.x - r.x, a.y - r.y, z);
	glm::vec3 p1(a.x + r.x, a.y + r.y, z);
	glm::vec3 p2(b.x + r.x, b.y + r.y, z);
	glm::vec3 p3(b.x - r.x, b.y - r.y, z);
	...
	if (isLeftViewport) {
		uint32_t base =
			static_cast<uint32_t>(connectionVertices2DLeft.size());

		connectionVertices2DLeft.push_back({ p0.x, p0.y, p0.z });
		connectionVertices2DLeft.push_back({ p1.x, p1.y, p1.z });
		connectionVertices2DLeft.push_back({ p2.x, p2.y, p2.z });
		connectionVertices2DLeft.push_back({ p3.x, p3.y, p3.z });

		for (uint32_t i : inds) {
			connectionIndices2DLeft.push_back(base + i);
		}
	}
	else {
		uint32_t base =
			static_cast<uint32_t>(connectionVertices2DRight.size());

		connectionVertices2DRight.push_back({ p0.x, p0.y, p0.z });
		connectionVertices2DRight.push_back({ p1.x, p1.y, p1.z });
		connectionVertices2DRight.push_back({ p2.x, p2.y, p2.z });
		connectionVertices2DRight.push_back({ p3.x, p3.y, p3.z });

		for (uint32_t i : inds) {
			connectionIndices2DRight.push_back(base + i);
		}
	}
}

void GeometryBuilder::rebuildPathGeometry2D(const std::vector<glm::ivec3>& path, bool isLeftViewport) {
	if (isLeftViewport) {
		connectionVertices2DLeft.clear();
		connectionIndices2DLeft.clear();
	}
	else {
		connectionVertices2DRight.clear();
		connectionIndices2DRight.clear();
	}

	for (size_t i = 0; i + 1 < path.size(); ++i) {
		glm::ivec2 a = glm::ivec2(path[i].x, path[i].y);
		glm::ivec2 b = glm::ivec2(path[i + 1].x, path[i + 1].y);
		createPathConnection2D(a, b, 0.12f, isLeftViewport);
	}
}